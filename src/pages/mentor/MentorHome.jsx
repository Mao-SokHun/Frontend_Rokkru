...
import { brandColors, isApiEnabled } from '@/constants'
...
const MentorHome = () => {
  ...
  const handleDeletePost = async (postId) => {
    ...
    if (!result?.ok) window.alert(t('mentorSchedule.publishFailed'))
  }
  ...
  return (
    <PageAmbient variant="mentor" className="space-y-5">
      {/* Start page header */}
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 text-white px-5 py-4 shadow-sm">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-primary-500/10 via-transparent to-transparent" aria-hidden />
              ...
              <AreaChart data={activityChart} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
                <defs>
                  <linearGradient id="actGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={brandColors.primary} stopOpacity={0.18} />
                    <stop offset="95%" stopColor={brandColors.primary} stopOpacity={0} />
                  </linearGradient>
                </defs>
                <XAxis dataKey="t" tick={{ fontSize: 11, fill: brandColors.chartMuted }} axisLine={false} tickLine={false} />
                <YAxis
                  allowDecimals={false}
                ...
                <Area
                  ...
                  fill="url(#actGrad)"
                  dot={false}
                  activeDot={{ r: 5, fill: brandColors.primary }}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
