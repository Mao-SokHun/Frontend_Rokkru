...
import { useAuth, useStudentEditProfile } from '@/hooks'
...
const StudentEditProfile = () => {
  ...
  const {
    ...
    provinceRows,
    provinceOptions,
    profileLoading: hookProfileLoading,
    profileSeed,
    saveProfile,
    deleteAccount,
    deletingAccount,
    deleteError: hookDeleteError,
    setDeleteError: setHookDeleteError,
  } = useStudentEditProfile(user, { t, lang, logout, updateUser })
  ...
  useEffect(() => {
    ...
    setProvince((current) =>
      resolveProvinceCanonicalName(current, provinceRows) || current
    )
  }, [provinceRows])

  useEffect(() => {
    setProfileLoading(hookProfileLoading)
  }, [hookProfileLoading])

  useEffect(() => {
    if (!profileSeed) return
    const merged = resolveStudentProfile({ ...user, ...profileSeed })
  ...
  const handleDeleteAccount = async () => {
    ...
    if (!result?.ok) setDeleteError(hookDeleteError || t('mentorProfile.deleteAccountFailed'))
  }
  ...
  return (
    <PageAmbient variant="ambient" className="space-y-6">
      ...
      >
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-4">
            <PageCard>
              <h3 className="font-bold text-slate-800 mb-4">{t('mentorProfile.personalInfo')}</h3>
              <NameFieldsGrid
                isKhmer={isKhmer}
                firstNameField={
                  <div>
            ...
            <PageCard className="border border-red-100 bg-red-50/40 space-y-3">
              <h3 className="font-bold text-red-800">{t('mentorProfile.dangerZone')}</h3>
              <p className="text-sm text-red-700">{t('mentorProfile.deleteAccountHint')}</p>
              <div>
                <button
                  type="button"
      ...
      >
        ...
        {(deleteError || hookDeleteError) && (
          <p className="mt-2 text-xs text-red-600">{deleteError || hookDeleteError}</p>
        )}
      </Modal>
    </PageAmbient>
  )
